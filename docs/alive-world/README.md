...
| [../../scripts/bot-ramp.sh](../../scripts/bot-ramp.sh) | `apply` one staged Min/Max bump + restart; `gates` dumps free/docker-stats/`compose ps`/online count/activity dials (optionally `--csv FILE [--note TXT]`); `wait [threshold] [timeout_sec]` blocks for online RNDBOT count; `csv FILE [--note TXT]` appends a CSV sample on its own |
