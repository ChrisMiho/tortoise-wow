...
|---|---|---|
| `scripts/bot-ramp.sh` | The ramp itself: `apply 400` sets Min/MaxRandomBots to one integer and restarts mangosd, `gates` dumps `free -h` + **Windows host free** + `docker stats` + `compose ps` + online count + the `AiPlayerbot.*` activity dials, `wait [threshold] [timeout_sec]` blocks until the **online-bot count** crosses the threshold, `csv FILE [--note TXT]` appends a CSV sample and `gates`/`csv` share the same writer. **`wait` is an online-count threshold, not plateau detection** — it returns `REACHED` the instant the count crosses, and nothing in it samples RSS. Bot inventory and talent construction continue well past login, so a row taken at `REACHED` sits on the rising edge and flattens the fit's slope while inflating its intercept, with nothing in the row to reveal it. After `REACHED`, take `csv` samples every ~2 min until `mangosd_rss_bytes` stops rising, *then* record the ramp point — see the handoff's §5. | **Fixed 2026-08-14:** `TW_STACK_ROOT` override, no longer drives the retired `tortoise-wow-v2` compose project, renamed off its previous leftover-task-number name, merged with the sibling script that used to duplicate its `wait` mode (now deleted from `scripts/` — its container-status handling is folded into `wait`), and gained a CSV mode via `gates --csv FILE [--note TXT]` or standalone `csv FILE [--note TXT]`, header written once per file so a whole ramp accumulates into one CSV. **Fixed 2026-08-15** (final review of this branch): a relative `--csv` path used to resolve against `$ROOT` because the script `cd`s there and never returns, so *every documented invocation lost its sample* to one error line buried under the gates dump — paths now resolve against the invoking cwd, the parent dir is created, and a sample that still cannot be written is dumped to stderr rather than lost; `gates` now prints host free (the tight stop gate) and not just the VM's; the schema is now **14 columns** (`schema_version,timestamp_utc,image_rev,target_bots,configured_bots,online_bots,mangosd_rss_bytes,mem_source,vm_total_bytes,vm_available_bytes,host_free_bytes,container_restarts,oom_killed,notes`) with a `schema_version` a reader can branch on, a `configured_bots` sampled from the conf so a `gates 400` after an `apply 400` that never took is visible in the data, and an append guard that refuses to write into a file whose header differs; `apply` now asserts both `sed`s landed before restarting mangosd; `wait` needs 3 consecutive non-running polls before `BLOCKED`, so a routine Docker-supervised restart no longer aborts it; and the bot count must be a plain integer. Remaining: nothing in the script — but read the `wait`/plateau note in the Role column before taking any ramp row. |
