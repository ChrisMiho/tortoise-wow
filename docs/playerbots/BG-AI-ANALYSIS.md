...
- Eye of the Storm (`BATTLEGROUND_EY`), Isle of Conquest (`BATTLEGROUND_IC`),
  Random BG (`BATTLEGROUND_RB`), arenas and vehicles are all compiled out. The live
  battlegrounds are **AV (1), WS (2), AB (3)** only.
...
| 6 | `:2723-2767` | `switch (bgType)` assigns them: AB → `vPaths_AB` + `vFlagsAB`; AV → `vPaths_AV` only, **`vFlagIds` left uninitialised**; WS → `vPaths_WS` + `vFlagsWS`; `default: break` leaves **both** uninitialised | An unhandled `bgType` leaves `vPaths` indeterminate, and it is dereferenced at `:2810`. Unreachable in this build (F-10). |
...
| **F-02** | **`wsgRoofJump` never uses `WS_FLAG_ALLIANCE_FLOOR_JUMP_LOWER`.** Line `:2642` repeats the `UPPER` constant where the Horde mirror at `:2633` correctly uses `LOWER`. `WS_FLAG_ALLIANCE_FLOOR_JUMP_LOWER`, declared at `:36`, has **zero references** in the file. | `BattleGroundTactics.cpp:2636-2643`; constants `:35-38`; Horde mirror `:2627-2634` | **broken** | A bot on the Alliance base's second floor (`x>1522, y<1468, z>361`) never receives the "drop into the flag room" waypoint — it is re-sent to the same upper ledge, with the jump flag set on an identical coordinate. That is the last hop of the **Horde attack route into the Alliance flag room**; its absence biases matches toward Alliance, which is the direction of the recorded 16-6 skew. | **medium** — the defect is unambiguous in source and the unused symmetric partner constant proves intent; the *link* to the skew is inference over 22 decisive matches. Raises to `high` by watching a Horde bot at ~`(1529, 1468, 362)` during a live match, or from a per-faction flag-pickup count in artifact 037. |
...
- **The 2.7:1 Alliance skew is not attributed.** F-02 points the right way, F-06 points
  ...
  boundary `pos.x > 933.f` and the Alliance mirror of that would be `1522.f`. Settling
