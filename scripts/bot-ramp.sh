...
# Pure shell + `tr`/`head` only: the runtime image is debian-slim and has no `ps`.
#
# Matches argv[0] from /proc/<pid>/cmdline. Two traps, both observed live on
# 2026-08-15 and both silent:
#
#   1. This read `/proc/<pid>/comm` until now, and comm for mangosd is
#      "MainThread" — the server renames its own main thread. So the lookup
#      NEVER matched, mangosd_pid() always returned empty, and
#      sample_mangosd_rss() fell through to its `docker stats` fallback for
#      every sample. That fallback is cache-inclusive, so it is the wrong
#      metric for a memory investigation; the row still looked healthy because
#      mem_source honestly said "dockerstats" and nobody had a reason to ask why.
#   2. Matching the WHOLE cmdline for "mangosd" reintroduces a worse bug: tini's
#      own cmdline is "/usr/bin/tini -g -- ./mangosd -c ...", so it contains
#      "mangosd" too, and /proc/1 sorts before /proc/7 in glob order. That
#      returns PID 1 and reports tini's ~1 MB as the world's RSS.
#
# Hence argv[0] only, and only its basename component.
mangosd_pid() {
